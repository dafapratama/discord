...
async def on_ready():
    ...
    print("Semua Pesan Akan Tersimpan pada {} Channel".format(log_channel))

    print(
        f'{client.user} is ready on server:\n'
        f'{guild.name}(id: {guild.id})'
    )

    members = '\n - '.join([member.name for member in guild.members])
    print(f'Anggota GUild:\n - {members}')

@client.event
async def on_member_join(member):
    await member.create_dm()
    await member.dm_channel.send(
        f'Hi {member.name}, Selamat datang Ke Discord Server!'
    )
...
def embed_card(event, msg, color=discord.Color.blue()):
    ...
    if event == "Deleted Message":
        ...
        if msg.attachments:
            ...
            file_type = msg.attachments[0].filename.split(".")
            img = "files/"+str(msg.attachments[0].id)+"."+file_type[1]
            result = scan(img)
            embed.add_field(
            ...
            file_type = msg.attachments[0].filename.split(".")
            img = "files/"+str(msg.attachments[0].id)+"."+file_type[1]
            result = sha256sum(img)
            embed.add_field(
                name="attachments sha256 hash",
                value=result,
                inline=True
            )
        ...
        result = hs.sha256(msg.content.encode("utf-8")).hexdigest()
        embed.add_field(
            name="message deleted sha256 hash",
            value=result,
            inline=False
        )
        embed.add_field(
        ...
        result3 = hs.sha256(msg[0].content.encode("utf-8")).hexdigest()
        result4 = hs.sha256(msg[1].content.encode("utf-8")).hexdigest()
        embed.add_field(
        ...
        embed.add_field(
            name="message before sha256 hash",
            value=result3,
            inline=False
        )
        embed.add_field(
            name="message after sha256 hash",
            value=result4,
            inline=False
        )
        embed.add_field(
...
def to_dictionary(msg):
    ...
    result = hs.md5(msg.content.encode("utf-8")).hexdigest()
    result3 = hs.sha256(msg.content.encode("utf-8")).hexdigest()
    mypath = None
    if msg.attachments:
    ...
    temp = dict(
        ...
        attachments= [{
            ...
            'sha256' : None if mypath == None else sha256sum(mypath)
        ...
        content= msg.content,
        content_hash_md5= result,
        content_hash_sha256= result3,
        created_at= created_at_format,
        content_before_edited = None if msg_before == None else msg_before,
        content_before_edited_md5hash= None if msg_before == None else hs.md5((msg.content).encode("utf-8")).hexdigest(),
        content_before_edited_sha256hash= None if msg_before == None else hs.sha256((msg.content).encode("utf-8")).hexdigest(),
...
def to_dataframe(msg):
    ...
    result4 = None if msg_before == None else hs.sha256(msg_before.encode("utf-8")).hexdigest()
    mypath = None
    if msg.attachments:
...
def donwload_file(message, path):
    ...
    fullfilename = os.path.join(mypath, str(message.attachments[0].id)+"."+file_type[1])
        ...
        if message.channel.name != "log2" and str(message.author.roles[1]) == 'admin':
            ...
            if "save" in message.content:
                if message.content == "save":
                    ...
                    messages = await message.channel.history(limit=9223372036854775807).flatten()
                ...
                elif "=" in message.content and message.content[-1] != "=":
                    ...
                    try:
                        ...
                        list_msg = list_msg[-val:]
                        msg_deleted = msg_deleted[-val:]
            ...
            if "csv" in message.content:
                if message.content == "csv":
                    print(msg_before)
                    messages2 = await message.channel.history(limit=9223372036854775807).flatten()
                    ...
                    columns = [
                        "id", "activity", "application", "content", "content_hash_md5", "content_hash_sha256", "created_at", "content_before_edited",
                        "content_before_edited_md5hash", "content_before_edited_sha256", "edited_at", "embeds",
                        "author_id", "author_name", "author_discriminator", "call", "clean_content", "jump_url", "mention_everyone",
                        "mentions", "pinned","nonce", "raw_channel_mention", "reactions", "reference", "raw_role_mention", "role_mention",
                        "stickers",
                    ...
                    try:
                        ...
                        list_msg2 = list_msg2[-val:]
                        list_msg_del = list_msg_del[-val:]
                        print(msg_before)
